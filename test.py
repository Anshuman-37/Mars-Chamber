...
class sensors:
    ...
    def set_sensors_port(self,ports):
        ...
        with concurrent.futures.ThreadPoolExecutor() as exe:
            humi_sensor = exe.submit(self.interim_set_ports,ports,'Vaisala',cntrl_val = 0);
            temp_sensor = exe.submit(self.interim_set_ports,ports,'AU{1}',cntrl_val = 1);
            pres_sensor = exe.submit(self.interim_set_ports,ports,'FT{1}',cntrl_val = 2);
    ...
    def set_sensor_data(self):
        ...
        '''
        with concurrent.futures.ThreadPoolExecutor() as exe:
            humi_sensor = exe.submit(self.connect_humi_sensor);
            temp_sensor = exe.submit(self.connect_table_temp_sensor); 
