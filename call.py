...
class Main:
    ...
    def OnOff_relay(self,target_temp):
        ...
        while True:
            time.sleep(1)
            try: 
                if(target_temp-2) > self.data['table_temp'][-1]:
                    GPIO.output(4, GPIO.HIGH) #turn relay 1 on
                    # if i % 100000 == 0:
                    print('Task still in execution')
                else: #(target_temp-1) < self.data['table_temp'][-1]:
                    GPIO.output(4, GPIO.LOW) # turn relay 0 off
                    print('This is done'); break; #return True;
                i = i+1 
            except:
                # if i % 100000 == 0:
                print('This shit is not working how it should work')
                i = i+1
                # return False;
    ...
    def concurent(self,ports,target_temp):
        ...
        with concurrent.futures.ThreadPoolExecutor() as exe:
            ...
            x = on_off.result(); sensor_values.result();
